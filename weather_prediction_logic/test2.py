...
def predictTemparatures(city = None):
    ...
    day_ctr = 0
    while day_ctr <= 6:
        predictedDailyData[days[day_ctr]] = {'min-temperature': min(current_day_dic.values()) + getRandomNegativeVariance(),'max-temperature': max(current_day_dic.values()) + getRandomPositiveVariance()}
        day_ctr += 1

    return latestData, output_data, predictedDailyData, city, state

latestData, predictedHourlyData, predictedDailyData, city, state = predictTemparatures("Ulhasnagar")
print(latestData)
print(predictedHourlyData)
print(predictedDailyData)